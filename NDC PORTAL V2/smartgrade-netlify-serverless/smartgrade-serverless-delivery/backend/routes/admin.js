...
router.post('/users/:userId/unlock', async (req,res,next)=>{try{const user=(await pool.query('SELECT id,failed_login_attempts,locked_until FROM users WHERE id=$1',[req.params.userId])).rows[0];if(!user)return res.status(404).json({error:'User not found.'});await pool.query('UPDATE users SET failed_login_attempts=0,locked_until=NULL,updated_at=now() WHERE id=$1',[user.id]);await audit(req,{action:'account_unlock',recordType:'user',recordId:user.id,previousValue:{failedLoginAttempts:user.failed_login_attempts,lockedUntil:user.locked_until},newValue:{failedLoginAttempts:0,lockedUntil:null}});res.json({message:'Account unlocked successfully. The existing password was not changed.'});}catch(e){next(e);}});
