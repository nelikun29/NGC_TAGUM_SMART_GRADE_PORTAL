...
router.get('/dashboard',async(req,res,next)=>{try{const q=async sql=>(await pool.query(sql)).rows[0].c;const [totalStudents,totalTeachers,totalClasses,activeUsers,pendingRegistrations,attendanceSessions,finalizedGrades,pendingUnfinalizeRequests]=await Promise.all([q(`SELECT COUNT(*)::int c FROM students`),q(`SELECT COUNT(*)::int c FROM users WHERE role='teacher' AND approval_status='approved'`),q(`SELECT COUNT(*)::int c FROM classes WHERE is_active=TRUE`),q(`SELECT COUNT(*)::int c FROM users WHERE is_active=TRUE`),q(`SELECT COUNT(*)::int c FROM users WHERE approval_status='pending'`),q(`SELECT COUNT(*)::int c FROM attendance_sessions`),q(`SELECT COUNT(*)::int c FROM grade_status WHERE status IN ('finalized','released')`),q(`SELECT COUNT(*)::int c FROM grade_unfinalize_requests WHERE status='pending'`)]);res.json({totalStudents,totalTeachers,totalClasses,activeUsers,pendingRegistrations,attendanceSessions,finalizedGrades,pendingUnfinalizeRequests});}catch(e){next(e);}});
router.get('/unfinalize-requests',async(req,res,next)=>{try{const teacherId=String(req.query.teacherId||'').trim();const status=String(req.query.status||'').trim();const params=[];const where=[];if(teacherId){params.push(teacherId);where.push('r.teacher_id={try{res.json((await pool.query(`SELECT * FROM academic_terms ORDER BY academic_year DESC`)).rows);}catch(e){next(e);}});
...
+params.length);}if(status){if(!['pending','approved','rejected','cancelled'].includes(status))return res.status(400).json({error:'Invalid request status.'});params.push(status);where.push('r.status={try{res.json((await pool.query(`SELECT * FROM academic_terms ORDER BY academic_year DESC`)).rows);}catch(e){next(e);}});
router.post('/terms',async(req,res,next)=>{try{const {academicYear,semester,setCurrent}=req.body;const id=crypto.randomUUID();if(setCurrent)await pool.query(`UPDATE academic_terms SET is_current=FALSE`);await pool.query(`INSERT INTO academic_terms(id,academic_year,semester,is_current) VALUES($1,$2,$3,$4)`,[id,academicYear,semester,!!setCurrent]);await audit(req,{action:'class_creation',recordType:'academic_term',recordId:id,newValue:{academicYear,semester}});res.status(201).json({id,academicYear,semester});}catch(e){next(e);}});
module.exports=router;
+params.length);}const{rows}=await pool.query(`SELECT r.*,c.subject,c.section,c.class_code,s.first_name AS student_first,s.last_name AS student_last,s.student_number,t.first_name AS teacher_first,t.last_name AS teacher_last,u.email AS teacher_email,reviewer.email AS reviewer_email FROM grade_unfinalize_requests r JOIN classes c ON c.id=r.class_id JOIN students s ON s.id=r.student_id JOIN teachers t ON t.id=r.teacher_id JOIN users u ON u.id=r.teacher_id LEFT JOIN users reviewer ON reviewer.id=r.reviewed_by ${where.length?'WHERE '+where.join(' AND '):''} ORDER BY CASE r.status WHEN 'pending' THEN 0 ELSE 1 END,r.requested_at DESC`,params);res.json(rows);}catch(e){next(e);}});
router.post('/unfinalize-requests/:requestId/approve',async(req,res,next)=>{const client=await pool.connect();try{const note=String(req.body.note||'').trim()||null;await client.query('BEGIN');const request=(await client.query(`SELECT * FROM grade_unfinalize_requests WHERE id=$1 FOR UPDATE`,[req.params.requestId])).rows[0];if(!request){await client.query('ROLLBACK');return res.status(404).json({error:'Unfinalize request not found.'});}if(request.status!=='pending'){await client.query('ROLLBACK');return res.status(409).json({error:'This request has already been reviewed.'});}const grade=(await client.query(`SELECT status FROM grade_status WHERE student_id=$1 AND class_id=$2 FOR UPDATE`,[request.student_id,request.class_id])).rows[0];if(!grade||!['finalized','released'].includes(grade.status)){await client.query('ROLLBACK');return res.status(409).json({error:'The learner grade is no longer finalized or released.'});}await client.query(`UPDATE grade_status SET status='in_progress',finalized_by=NULL,finalized_at=NULL,released_at=NULL WHERE student_id=$1 AND class_id=$2`,[request.student_id,request.class_id]);await client.query(`UPDATE grade_unfinalize_requests SET status='approved',reviewed_by=$1,reviewed_at=now(),admin_note=$2 WHERE id=$3`,[req.user.id,note,request.id]);await client.query('COMMIT');await audit(req,{action:'grade_unfinalize_approved',recordType:'grade_unfinalize_request',recordId:request.id,previousValue:grade.status,newValue:'in_progress'});res.json({message:'Request approved. The learner grade is now In Progress and can be corrected by the teacher.'});}catch(e){try{await client.query('ROLLBACK');}catch{}next(e);}finally{client.release();}});
router.post('/unfinalize-requests/:requestId/reject',async(req,res,next)=>{const client=await pool.connect();try{const note=String(req.body.note||'').trim();if(note.length<3)return res.status(400).json({error:'Please provide a brief reason for rejecting the request.'});await client.query('BEGIN');const request=(await client.query(`SELECT * FROM grade_unfinalize_requests WHERE id=$1 FOR UPDATE`,[req.params.requestId])).rows[0];if(!request){await client.query('ROLLBACK');return res.status(404).json({error:'Unfinalize request not found.'});}if(request.status!=='pending'){await client.query('ROLLBACK');return res.status(409).json({error:'This request has already been reviewed.'});}await client.query(`UPDATE grade_unfinalize_requests SET status='rejected',reviewed_by=$1,reviewed_at=now(),admin_note=$2 WHERE id=$3`,[req.user.id,note,request.id]);await client.query('COMMIT');await audit(req,{action:'grade_unfinalize_rejected',recordType:'grade_unfinalize_request',recordId:request.id,newValue:{status:'rejected',note}});res.json({message:'Unfinalize request rejected. The learner grade remains locked.'});}catch(e){try{await client.query('ROLLBACK');}catch{}next(e);}finally{client.release();}});
