...
async function attendancePercent(studentId,classId,maxDays) {
  ...
  // Fixed semester denominator: recording Present changes only earned days,
  // never Total Required Days. Excused records do not add earned points.
  const points=rows.reduce((sum,r)=>sum+(r.status==='present'?1:r.status==='late'?0.75:0),0);
  return {percent:Math.max(0,Math.min(100,(points/denominator)*100)),complete:true,earned:points,max:denominator};
}
...
function summarize(rows,method='average_percentage') {
  ...
  // Every assessment retains its configured fixed maximum. A learner score
  // never becomes the denominator. This applies to quiz/PT/exam/custom.
  ...
  if(method==='points_total'){
    ...
    return {percent:(raw/max)*100,complete:true,earned:raw,max};
  }
...
async function componentResult(studentId,classId,c){
  ...
  if(c.subcomponents&&c.subcomponents.length){
    ...
    for(const sub of c.subcomponents){
      let rows;
      if(c.source_type==='custom')rows=(await pool.query(`SELECT s.raw_score,a.max_score,s.verification_status FROM custom_assessment_scores s JOIN custom_assessments a ON a.id=s.assessment_id WHERE a.class_id=$1 AND a.component_id=$2 AND a.subcomponent_id=$3 AND s.student_id=$4`,[classId,c.id,sub.id,studentId])).rows;
      else rows=await assessmentRows(studentId,classId,c.source_type,sub.source_filter||sub.name);
      const r=summarize(rows,c.calculation_method);if(!r.complete||r.percent===null)return r;
...
async function computeDynamicClassGrade(studentId,classId){
  ...
  const components={},missing=[];let finalGrade=0;
  for(const c of scheme.components){
    const r=await componentResult(studentId,classId,c);
    components[c.component_key]={name:c.name,percent:r.percent,weight:Number(c.weight),available:r.complete&&r.percent!==null,adjusted:false,earned:r.earned,max:r.max};
  ...
  if(missing.length)return {complete:false,status:status?status.status:'in_progress',missing,components,finalGrade:null,message:`Grade Incomplete — ${missing.map(k=>components[k]?.name||k).join(', ')} score/configuration pending.`,schemeId:scheme.id,schemeVersion:scheme.version};
