...
async function computeDynamicClassGrade(studentId,classId){
  ...
  for(const c of scheme.components){const r=await componentResult(studentId,classId,c);components[c.component_key]={name:c.name,percent:r.percent,weight:Number(c.weight),available:r.complete&&r.percent!==null,adjusted:false,earned:r.earned,max:r.max};if(Number(c.weight)>0&&(!r.complete||r.percent===null))missing.push(c.component_key);else if(Number(c.weight)>0)finalGrade+=r.percent*Number(c.weight)/100;}
