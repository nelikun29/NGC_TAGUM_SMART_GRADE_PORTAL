...
CREATE TABLE IF NOT EXISTS grading_schemes (
  ...
  name TEXT NOT NULL DEFAULT 'Class Grading Scheme',
  state TEXT NOT NULL DEFAULT 'draft' CHECK (state IN ('draft','active','finalized')),
...
CREATE TABLE IF NOT EXISTS grading_subcomponents (
  id TEXT PRIMARY KEY,
  component_id TEXT NOT NULL REFERENCES grading_components(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  subcomponent_key TEXT NOT NULL,
  weight_share NUMERIC(7,4) NOT NULL CHECK (weight_share >= 0 AND weight_share <= 100),
  source_filter TEXT,
  sort_order INTEGER NOT NULL DEFAULT 0,
  is_active BOOLEAN NOT NULL DEFAULT TRUE,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE(component_id, subcomponent_key)
);
...
CREATE TABLE IF NOT EXISTS custom_assessments (
  ...
  subcomponent_id TEXT REFERENCES grading_subcomponents(id) ON DELETE SET NULL,
  title TEXT NOT NULL,
  description TEXT,
...
CREATE TABLE IF NOT EXISTS custom_assessment_scores (
  id TEXT PRIMARY KEY,
  assessment_id TEXT NOT NULL REFERENCES custom_assessments(id) ON DELETE CASCADE,
...
-- IMPORTANT: application code must validate that active top-level component
-- weights total exactly 100 and, where subcomponents exist, their weight_share
-- values total exactly 100. PostgreSQL CHECK constraints cannot safely enforce
-- cross-row totals by themselves.
